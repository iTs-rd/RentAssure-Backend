...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        exclude = ['password', 'user_permissions', 'groups', 'is_active',
                   'is_staff', 'start_date', 'is_superuser', 'last_login']
...
class UserSerializerPassword(serializers.ModelSerializer):
    class Meta:
        model = UserModel
        exclude = ['dp', 'user_permissions', 'groups',
                   'is_active', 'is_staff', 'start_date', 'last_login']
        extra_kwargs = {'password': {'write_only': True, 'required': True}}
...
class DataMiniSerializer(serializers.ModelSerializer):
    class Meta:
        model = DataModel
        fields = ('id', 'user', 'property_type', 'img1', 'img2', 'img3', 'img4', 'title', 'area',
